...
import { hexToRgba, orderEffects } from '../../awa/awa.common.utils';
import { EFFECTOR_GROUP_ID_BODY } from '../../awa/awa.constants';
import { ELEMENT_EFFECTS } from '../../awa/awa.core';
import SVG from './svg';
...
const updateFunctions = {
  ...
  // DropShadow effect
  dropShadow: getAttrSetter(['in', 'dx', 'dy', 'stdDeviation', 'flood-color']),
  // Flood effect
...
extend(Element, {
  ...
  filterWith: function (block) {
    ...
    var thisFilters = this.attr('filter') || '';
    thisFilters += (filter+' ');

    return this.attr('filter', thisFilters)
  },
  // Remove filter
  ...
  effector : function() {
      ...
      var svgInstance = _self.node.ownerSVGElement.instance;
      var awa = svgInstance._awa;

      function getEffectorGroup(_elemId)
      {
         var effectorGroupId = _elemId+EFFECTOR_GROUP_ID_BODY;

         var effectorGroup = svgInstance.findOne("#"+effectorGroupId);

         if(!effectorGroup) // create
         {
          effectorGroup = svgInstance.group().attr({id : effectorGroupId});
          
          var selectedSceneContainer = awa.getActiveSceneContainer();
          
          selectedSceneContainer.add(effectorGroup)
          effectorGroup.insertAfter(_self)

         }

         return effectorGroup;

      }

      // Check if group of effectors exist or not then create

      function createUseElement(_id)
      {
        var useElement = svgInstance.use(_self).attr('id',_id+'-use');
        useElement.addClass("awa--no--event")

        var effectorGroup = getEffectorGroup(_self.attr("id"));
        effectorGroup.add(useElement);

        return useElement;
      }


      function dropShadow(_effect = {properties : {x:5, y:5, blur:0, spread:0, color:'#000'}})
      {
       
          var useElement = createUseElement(_effect.id);
            
            // var w = useElement.width();
            // var h = useElement.width();
            var rgba = hexToRgba(_effect.properties.color);
            var matrixValues=
            `0 0 0 ${rgba.r} 0 
             0 0 0 ${rgba.g} 0 
             0 0 0 ${rgba.b} 0 
            ...
            var ef = svgInstance.filter();
            
            ef
            .colorMatrix('matrix', matrixValues).result('opaque-source')
            .gaussianBlur(".85")
            .offset(_effect.x, _effect.y).result("shadow")
            .composite('shadow', 'atop', '').in(ef.$source)


            ef.size(("2000%"),("2000%")).move("-1000%","-1000%")

            ef.attr({id:_effect.id})
            ef.m_name = 'drop shadow';
            
            useElement.filterWith(ef);

      }
      
      function innerShadow(_effect = {properties :{x:5, y:5, blur:0, spread:0, color:'#000'}})
      {
          ...
          ef.offset((_effect.properties.x), (_effect.properties.y)).result("offset-shadow")
          .gaussianBlur(_effect.properties.blur).in("offset-shadow").result("offset-blur")
          .composite("offset-blur", 'out').in(ef.$source).result('inverse')
          .flood(_effect.properties.color, 1).result('color')
          .composite('inverse', 'in', '').in('color')

          ef.attr({id:_effect.id})
          ef.m_name = 'inner shadow';
      ...
      function blur(_effect = {properties : {blur:5}})
      {
          // We don't create an use element for the case of blur
          // The effect is directly added to the original element

          var ef = svgInstance.filter();

          ef.gaussianBlur(_effect.properties.blur)
      ...
      function inset(_effect = {properties : {depth:15, color:"#000000"}})
      {
          var useElement = createUseElement(_effect.id);
          ...
          var ef = svgInstance.filter();
            ...
            ef
            .colorMatrix('matrix', matrixValues).result('opaque-source')
            .gaussianBlur(_effect.properties.depth)
            .composite('opaque-source', 'xor').in('')
            .composite('opaque-source', 'in').in('')
      ...
      function chainEffects(_effects = [])
      {
        _effects = orderEffects(_effects); // Order to avoid unwanted overlaying

        if(!_effects.length) return;
        
          for (let i = 0; i < _effects.length; i++) 
          {
            const _effect = _effects[i];
...
const chainingEffects = {
  ...
  // DisplacementMap effect
  dropShadow: function (x, y, stdDeviation, floodColor) {
    return this.parent() && this.parent().dropShadow(this, x, y, stdDeviation, floodColor).in(this) // pass this as the first input
  },
  // Flood effect
