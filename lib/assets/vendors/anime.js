...
const validTransforms = ['translateX', 'translateY', 'translateZ', 'rotate', 'rotateX', 'rotateY', 'rotateZ', 'scale', 'scaleX', 'scaleY', 'scaleZ', 'skew', 'skewX', 'skewY', 'perspective', 'matrix', 'matrix3d', 'followPath', 'morphTo'];

const strokeTransforms = {'strokeDasharray' : 'stroke-dasharray', 'strokeDashoffset':'stroke-dashoffset'};

const validEffectAttributes = {'dx':'dx', 'dy':'dy', 'depth':'depth', 'blur':'blur', 'flood':'flood'};
...
function getAnimationType(el, prop) {
  if (is.dom(el) && !arrayContains(Object.keys(validEffectAttributes), prop) && !is.inp(el) && (!is.nil(getAttribute(el, prop)) || (is.svg(el) && el[prop]))) return 'attribute'; // blur is an svg attr
  if (is.dom(el) && arrayContains(Object.keys(strokeTransforms), prop)) return 'attribute'; // stroke... attributes
  if (is.dom(el) && arrayContains(validTransforms, prop)) return 'attribute'; // Old : transform
  if (is.dom(el) && arrayContains(Object.keys(validEffectAttributes), prop))  return 'effect'; // only for effect
...
function getPath(path, percent, centered=true, rotate=true) {
...
function normalizeTweens(prop, animatable) {
  ...
  return prop.tweens.map(t => {
    ...
    const _tweenValue = tween.value;
    let to = is.arr(tweenValue) ? tweenValue[1] : tweenValue;
    let _toPath = _tweenValue;
    ...
    const _fromPath = !_previousTween ? normalizeTweenValues(prop.tweens[0], animatable).value : _previousTween;
    ...
    previousTween = tween;
    _previousTween = _toPath;
    if(prop.name == 'morphTo')
    { // d values are computed otherwise they change over tweening and will cause unwanted behaviour
      tween.fromPath = {path : _fromPath, d : _fromPath.el.getAttribute("d")};
      tween.toPath = {path : _toPath, d : _toPath.el.getAttribute("d")};
    }
...
const setProgressValue = {
  ...
  attribute: (t, p, v, transforms, manual) => 
  {
    if(!validTransforms.includes(p))
    {
      if(arrayContains(Object.keys(strokeTransforms), p)) p = strokeTransforms[p]; // Get valid attribute value

      t.setAttribute(p, v)
    }
    else if(p == 'followPath')
    {
      var tx = v.x, 
          ty = v.y, 
          angle = v.angle, 
      ...
      var cx = t.getBBox().x + t.getBBox().width / 2;
      var cy = t.getBBox().y + t.getBBox().height / 2;

      var rotateStr = '';
      if(!centered) {cx = 0; cy = 0;}
      if(rotate) 
      {
        rotateStr = `rotate(${angle}, ${cx}, ${cy})`;
      }

      var str = `translate(${tx-cx}, ${ty-cy}) ${rotateStr}`;
      t.setAttribute("transform", str);

    }
    else if(p == 'morphTo')
    {
      var str = `${v}`;
      t.setAttribute("d", str);
    }
    else                  // Give a specific value for rotation
    {
      transforms.list.set(p, v);
      var tArray = Array.from(transforms.list.entries());

      var hasTranslate = (tArray.find(t=>t[0] == 'translateY') || tArray.find(t=>t[0] == 'translateX')); 
      {
        let str = '';
        transforms.list.forEach((value, prop) => { 
          if(prop == 'rotate')
          {
            ...
            str += `rotate(${value}, ${cx}, ${cy}) `;

          }
          else if (hasTranslate) // translateX and translateY (translateZ)
          {
            // Check for translateY value
            // console.log(transforms.list)

            if(prop == 'translateX')
            {
              var tYItem = tArray.find(t=>t[0] == 'translateY')
              var tYValue = 0;
              if(tYItem)
                tYValue = tYItem[1];

              // Check for scale too...
              var tScaleXItem = tArray.find(t=>t[0] == 'scaleX')
              var tScaleYItem = tArray.find(t=>t[0] == 'scaleY')
              var tScaleXValue = 1;
              var tScaleYValue = 1;

              // Used with translateX/Y to apply the scaling at the right anchor pos
              var tScaleXDiff = 0;
              var tScaleYDiff = 0;
              // Getting the anchor from the svgjs element
              var anchorX = 0.5;//t.anchor.x;
              var anchorY = 0.5;//t.anchor.y;
              // Scaling point
              var cx = t.getBBox().x + t.getBBox().width * anchorX;
              var cy = t.getBBox().y + t.getBBox().height * anchorY;

              var tScaleStr = '';

              if(tScaleXItem)
              {
                tScaleXValue = tScaleXItem[1];

                if(tScaleYItem)
                {
                  tScaleYValue = tScaleYItem[1];
                  tScaleYDiff = cy * Math.abs(1-tScaleYValue);
                }

                tScaleXDiff = cx * Math.abs(1-tScaleXValue);
              ...
              tScaleStr = `scale(${tScaleXValue}, ${tScaleYValue}) `

              str += `translate(${value-tScaleXDiff}, ${tYValue-tScaleYDiff}) ${tScaleStr} `;
            }
            else if(!(tArray.find(t=>t[0] == 'translateX'))) // Only Y axis tweens
            {
          ...
          else if (!hasTranslate)
          {
            ...
            var tScaleYDiff = 0;
            // Getting the anchor from the svgjs element
            var anchorX = t.anchor.x;
            var anchorY = t.anchor.y;
            // Scaling point
            var cx = t.getBBox().x + t.getBBox().width * anchorX;
            var cy = t.getBBox().y + t.getBBox().height * anchorY;
            ...
            else if(tScaleYItem && !tScaleXItem)
            {
              tScaleYValue = tScaleYItem[1];
              tScaleYDiff = cy * Math.abs(1-tScaleYValue);
            }
            ...
            if(tScaleXItem)
            {
              ...
              str += `translate(${-tScaleXDiff}, ${-tScaleYDiff}) ${tScaleStr} `;
              // console.log("scaleX zone")
            }
            else if((tScaleYItem && !tScaleXItem))
            {
              tScaleStr = `scale(${tScaleXValue}, ${tScaleYValue}) `
  ...
  effect: (t, p, v, transforms, manual) => {

    // Get child to animate
    switch(p)
    {
      case validEffectAttributes.dx : 
        // Get real target
        var childTarget = childTarget || t.querySelector("feOffset");
        childTarget.setAttribute(validEffectAttributes.dx, v)

      break;

      case validEffectAttributes.dy : 
        // Get real target
        var childTarget = t.querySelector("feOffset");
        childTarget.setAttribute(validEffectAttributes.dy, v) 
      break;

      case validEffectAttributes.blur : 
        // Get real target
        var childTarget = t.querySelector("feGaussianBlur");
        childTarget.setAttribute('stdDeviation', v) // double value in case we wish later to support h/v blur
      break;

      case validEffectAttributes.depth : 
        // Get real target
        var childTarget = childTarget || t.querySelector("feGaussianBlur");
        childTarget.setAttribute('stdDeviation', v)  
      break;


      case validEffectAttributes.flood : 
      // Get real target
        var childTarget = childTarget || t.querySelector("feColorMatrix") || t.querySelector("feFlood");

        var _attr = 'flood-color';
        if(childTarget.hasAttribute('values')) // color matrix then
        {
          _attr = 'values';
          var colorBase = 255;
          var rgba = v.replace(/^rgba?\(|\s+|\)$/g,'').split(',');

          var matrixValues=
           `0 0 0 ${rgba[0]/colorBase} 0 
            0 0 0 ${rgba[1]/colorBase} 0 
            0 0 0 ${rgba[2]/colorBase} 0 
            0 0 0 1 0`;

          v = matrixValues;
        }

        childTarget.setAttribute(_attr, v)  
...
function anime(params = {}) {
  ...
  function setAnimationsProgress(insTime) {
    ...
    while (i < animationsLength) {
      ...
      for (let n = 0; n < toNumbersLength; n++) {
        ...
        if (!tween.isPath && anim.property != "followPath" && anim.property != "morphTo") {
          value = fromNumber + (eased * (toNumber - fromNumber));
        }
        else if(anim.property == "followPath")
        {
          value = getFollowPathValue(tween, (eased * toNumber))
        }
        else if(anim.property == "morphTo")
        {
      ...
      if(anim.property != "followPath" && anim.property != "morphTo")
      {
        if (!stringsLength) {
          progress = numbers[0];
        } else {
        ...
        else if(anim.property == "morphTo")
        {
          ...
          var tweenDuration = tween.duration;
          var t = eased * (insTime / tweenDuration);

          var fromPath = tween.fromPath.d; 
          var toPath = tween.toPath.d;

          var interpolator = interpolate(fromPath, toPath)
  ...
  function getFollowPathValue(followPathTween, progress)
  {
    var path = followPathTween.value.el;
    var centered = followPathTween.value.centered; // Whether the element is centered on path or not
    var rotate = followPathTween.value.rotate; // Whether the element must be rotated or not
    function point(offset = 0) {
      const l = progress + offset >= 1 ? progress + offset : 0;
    ...
    var point = {x:p.x, y:p.y, angle : angle, rotate, centered }
    return point;
  }
  ...
  function getInterpolationPrevTween(tweens, index)
  {
    var _prevIndex = index-1;
    var prevIndex = _prevIndex >=1 ? _prevIndex : 0;

    return tweens[prevIndex];
  }

  // Added by @Habibe to fix path interpolation issue
  function interpolatePath(path1, path2, progress) {
  
    

  // var path1 = _path1;//.value.el.getAttribute("d");
  // var path2 = _path2;//.value.el.getAttribute("d");

  const shape1 = path1.match(/[a-df-z][^a-df-z]*/ig) || [];
  const shape2 = path2.match(/[a-df-z][^a-df-z]*/ig) || [];
  const length = Math.min(shape1.length, shape2.length);
  let interpolatedPath = '';
  
  for (let i = 0; i < length; i++) {
    const initialCommand = shape1[i].charAt(0);
    const initialValues = shape1[i].substring(1).split(/[ ,]/).map(parseFloat);
    const finalValues = (shape2[i] || '').substring(1).split(/[ ,]/).map(parseFloat);
    const command = initialCommand;

    if (initialCommand === 'M' && i > 0) {
      interpolatedPath += 'L'; // Treat subsequent pairs as line commands
    } else {
      interpolatedPath += command; // Keep the initial command for the first pair
    }

    for (let j = 0; j < initialValues.length; j++) {
      const initialValue = initialValues[j] || 0;
      const finalValue = finalValues[j] || 0;
      const interpolatedValue = initialValue + (finalValue - initialValue) * progress;
      
      // Convert to fixed precision if not an integer
      const formattedValue = Number.isInteger(interpolatedValue) ? interpolatedValue : interpolatedValue.toFixed(2);
      
      interpolatedPath += formattedValue;
    ...
    var cleanMorphedPath = interpolatedPath.trim();
    // cleanMorphedPath.split(" 0.00 ").join(" ");
    // cleanMorphedPath.split(" 0 ").join(" ");
    // cleanMorphedPath = cleanMorphedPath.split("M0").join("M");
    // cleanMorphedPath = cleanMorphedPath.split("C0.00").join("C");
    // cleanMorphedPath = cleanMorphedPath.split("0C ").join("C");
    // cleanMorphedPath = cleanMorphedPath.split("0 C ").join("C");
    // cleanMorphedPath = cleanMorphedPath.split("C0").join("C");

    const patternsToReplace = ['M0', '0C', '0 C', 'C0'];

    // Create a regular expression pattern by joining the substrings with "|" (OR) for multiple patterns
    const pattern = new RegExp(patternsToReplace.join('|'), 'g');
  
    // Use the replace method with a function to replace occurrences with only the letter
    cleanMorphedPath = cleanMorphedPath.replace(pattern, match => match.charAt(1));
  

    return cleanMorphedPath;
  }
...
anime.getDashoffset = getDashoffset;
