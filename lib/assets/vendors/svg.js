...
*/
import { initBg } from "../../awa/awa.common.utils";
import { DEFBASE_ID_BODY, DEFBG_ID_BODY, GRAD, GRADIENT_ID_BODY, GRADIENT_ID_BODYIENT_ID_BODY, MEDIA_RENDER_MODE, PATTERN_BG_ID_BODY } from "../../awa/awa.constants";
import { TIMELINE_MAX_DURATION } from "../../awa/awa.core";
...
var SVG = (function () {
  ...
  **/
  ...
  const html = 'http://www.w3.org/2000/xhtml';
  const xmlns = 'http://www.w3.org/2000/xmlns/';
  const xlink = 'http://www.w3.org/2000/xlink';
  ...
  function eid(name) {
    return 'awa-' + capitalize(name) + did++;
  ...
  class Dom extends EventTarget {
    ...
    revoke(element) {
      ...
      element.canvasOwnerId(null); // Set canvas parent to null

      this.node.ownerSVGElement.appendChild(element.node);
      return this;
    } // Add element to given container and return self
    ...
    hasChild(_el){
      return this.node.querySelector('#'+_el.attr("id")) ? true : false;
    }

    getDefs()
    {
      // return this.findOne("defs") ? this.findOne("defs") : null;
      var defsNode = this.node.querySelector("defs") ? this.node.querySelector("defs").instance : null;
    ...
    getParentingDefs()
    {
      var parentingDefsNode = this.parent().type == "defs" ? this.parent() : null;
      // console.log(parentingDefsNode);
      return parentingDefsNode;
    }
  ...
  class Element extends Dom {
    constructor(node, attrs) {
      ...
      this.m_name = this.node.localName;
      this.m_path = null;
      this.m_pathString = null;

      this.node.parentId = null; // parent Id
      this.node.canvasOwnerId = null; // parent Id
      this.node.anchor = {x : 0.5, y : 0.5};
      this.node.baseFill = {id:null, color : '#ffffff'};
      this.node.baseStroke = {id:null, color : '#000000'};
      this.node.baseBackground = {id : null, type:null, media : null, mode : MEDIA_RENDER_MODE.fill, tile : 4};
      this.node.effects = [];

      // this.node.clips = []; // custom animations for this element
      // This element timeline
      this.node.animationTimeline = anime.timeline({
        easing: "linear",
    ...
    {
      if(arguments.length == 0)
        return this.node.canvasOwnerId;

      this.node.canvasOwnerId = id

      return this;
    
    }

    baseFill(baseId, baseFill)
    {
      if(arguments.length == 0)
        return this.node.baseFill;

      this.node.baseFill = {id:baseId, color : baseFill};
      
      return this;
    }

    baseStroke(baseId, baseStroke)
    {
      if(arguments.length == 0)
        return this.node.baseStroke;

      this.node.baseStroke = {id:baseId, color : baseStroke};

      return this;
    }
    
    baseBackground(baseId, baseType, baseMedia, renderMode=MEDIA_RENDER_MODE.fill, tile = 4)
    {
      if(arguments.length == 0)
        return this.node.baseBackground;

      this.node.baseBackground = {id : baseId, type:baseType, media : baseMedia, mode:renderMode, tile:tile};
    ...
    removeEffect(_id)
    {
      var svgInstance = this.node.ownerSVGElement.instance;

      var effect = svgInstance.findOne("#"+_id);

      if(effect)
      {
        effect.remove();
      }
      else // Then blur : remove the filter from the node
      {
        this.unfilter()
      }
    ...
    {
      if(arguments.length == 0)
        return this.node.clips;

      this.node.clips = _clips;
    ...
    addClip(_clip)
    {
      this.node.clips.push(_clip);
      return this;
    }

    removeClip(_id)
    {
      var clipIndex = this.node.clips.findIndex(c=>c.name == _id);
      if(clipIndex == -1) return this;
      this.node.clips.splice(clipIndex, 1);
      return this;
    }
    ...
    {
      if(arguments.length == 0)
        return this.node.animationTimeline;

      this.node.animationTimeline = _animationTimeline;
    ...
    canvasContainer(o) {
      ...
      this._canvasContainer = o;
      return this;
    } // Set _canvasContainer or get it


    setGradientElement(_colorType=null, _type=null, _angle=null, _values=null)
    {
        var _elemId = this.attr('id');

        var svgInstance = this.node.ownerSVGElement.instance;

        var gradientId = _elemId+GRADIENT_ID_BODY+"--"+_colorType //+"--"+new Date().getTime();
        ...
        // if(!gradient) // create
        {
          ...
          var gradient = svgInstance.gradient(_type).attr({id : gradientId});

          for (let i = 0; i < _values.length; i++)
          {
            const _vals = _values[i];
            var stopPoint = parseInt(_vals.percentage)/100;
            gradient.stop(stopPoint, _vals.color)
          }

          if(_type == 'linear')
          {
            const radians = parseInt(_angle) * Math.PI / 180; // Convert degrees to radians
            const x1 = 0.5 + 0.5 * Math.cos(radians); // x1 = 0.5 + 0.5 * cos(angle)
            const y1 = 0.5 + 0.5 * Math.sin(radians); // y1 = 0.5 + 0.5 * sin(angle)
            const x2 = 0.5 - 0.5 * Math.cos(radians); // x2 = 0.5 - 0.5 * cos(angle)
            const y2 = 0.5 - 0.5 * Math.sin(radians); // y2 = 0.5 - 0.5 * sin(angle)
        ...
        gradientId = gradientId.split(DEFBASE_ID_BODY).join("") // remove the appended baseId
    ...
    setBackgroundElement(_mediaType=null, _media=null, _mode=MEDIA_RENDER_MODE.fill, _tile=4)
    {
        var _elemId = this.attr('id');

        _elemId.split(DEFBG_ID_BODY).join("")

        var svgInstance = this.node.ownerSVGElement.instance;

        var backgroundId = _elemId+PATTERN_BG_ID_BODY;

        var backgroundPattern = svgInstance.findOne("#"+backgroundId);

        if(backgroundPattern) backgroundPattern.remove();

        backgroundPattern = svgInstance.pattern(20, 20, function(add) {
          
          var image = add.image()
           .attr({width:20, height:20})
            
           image.node.removeAttribute("href")
           image.node.setAttribute("href" , _media)

         }).attr({id:backgroundId})


        this.fill(backgroundPattern);
  ...
  const methods = ['click', 'dblclick', 'mousedown', 'mouseup', 'mouseover', 'mouseout', 'mousemove', 'mouseenter', 'mouseleave', 'touchstart', 'touchmove', 'touchleave', 'touchend', 'touchcancel', 'drop', 'dragover'].reduce(function (last, event) {
  ...
  ***/
  ...
  class Path extends Shape {
    ...
    array() {
      return /* this._array ||  */(this._array = new PathArray(this.m_pathString));
  ...
  class Svg extends Container {
    ...
    // @Shad : Rewrote defs method to be able to create multiple defs across the dom
    defs(_current=true) {
  ...
  function dmove(dx, dy) {
    ...
    this.attr({transform : translation})  
  ...
  class Use extends Shape {
    ...
    use(element, file) {
      // Set lined element
      return this.attr('href', (file || '') + '#' + element, '');
    }
...
// export function createSVGInstance(elementId) {
//   return new SVG(elementId);
// }
