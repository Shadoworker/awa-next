        ...
        if(this.m_loonkInstance._awa.isElementDraggingStateDragging())
        {
            ...
            if(elem.instance && isCanvasContainer(elem.instance))
            {
              ...
              if(isPointInCanvas(point,elem)) // Insert
              {
                if(!canvasItemsGroup.hasChild(draggedElement))
                {
                  ...
                  var elementRefBgId = '#'+draggedElement.attr("id")+DEFBG_ID_BODY;
                  var elementRefBaseId = '#'+draggedElement.attr("id")+DEFBASE_ID_BODY;
                  
                  var elementRefBg = this.m_loonkInstance._awa.getSvgInstance().findOne(elementRefBgId);
                  var elementRefBase = this.m_loonkInstance._awa.getSvgInstance().findOne(elementRefBaseId);

                  canvasItemsGroup.add(elementRefBg);
                  canvasItemsGroup.add(elementRefBase);
                  // canvasItemsGroup.add(elementRefsGroup);

                  // canvasItemsGroup.add(draggedElement);
                  // Add the element to new defs bloc
                  var elementDefs = this.m_loonkInstance._awa.m_svgInstance.defs(false);
                  canvasItemsGroup.add(elementDefs);
                  elementDefs.add(draggedElement)
          ...
          if( draggedElement.hasClass(INCANVAS_ITEM_CLASS)) // Is in a canvas
          {
            ...
            if(!isPointInCanvas(point,canvasContainer.node) && canvasItemsGroup) 
            {
              ...
              canvasItemsGroup.revoke(elementRefsGroup);
