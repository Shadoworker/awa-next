...
import { CLIP_ID_BODY, CONTAINER_ID_BODY, DEFBASE_ID_BODY, DEFBG_ID_BODY, DEFREFS_GROUP_ID_BODY, EFFECTOR_GROUP_ID_BODY, GROUP_ID_BODY, INCANVAS_ITEM_CLASS, PATTERN_BG_ID_BODY, PLUGGER_ACTIVE_CLASS } from '../../awa/awa.constants';
    ...
    _createClass(DragHandler, [{
      ...
      value: function startDrag(ev) {
        ...
        if(this.m_loonkInstance.isSelectMenuContext() || this.m_loonkInstance.isEditMenuContext())
        {
          ...
          // If has background
          var patternBgId = this.el.attr("id")+DEFBG_ID_BODY+PATTERN_BG_ID_BODY;

          if(this.m_loonkInstance._awa.getSvgInstance().findOne("#"+patternBgId))
          {
            var lastPBgTransform = this.m_loonkInstance._awa.getSvgInstance().findOne("#"+patternBgId).transform();
            this.lastPBgTranslate = {x:lastPBgTransform.translateX, y:lastPBgTransform.translateY}
          }
      ...
      value: function drag(ev) {
        ...
        else
        {
          ...
          if(this.m_loonkInstance._awa.getSvgInstance().findOne("#"+patternBgId))
          {
            var patternBg = this.m_loonkInstance._awa.getSvgInstance().findOne("#"+patternBgId);

            var pbgdx = (dx + this.lastPBgTranslate.x);
            var pbgdy = (dy + this.lastPBgTranslate.y);
          
            var translation = `translate(${pbgdx}, ${pbgdy})`
    
            patternBg.attr({transform : translation}) 
        ...
        if(this.m_loonkInstance._awa.isElementDraggingStateDragging())
        {
            ...
            if(elem.instance && isCanvasContainer(elem.instance))
            {
              ...
              if(isPointInCanvas(point,elem)) // Insert
              {
                if(!canvasItemsGroup.hasChild(draggedElement))
                {
                  ...
                  // Get and parent the element def refs
                  var elementRefsGroupId = '#'+draggedElement.attr("id")+DEFREFS_GROUP_ID_BODY;
                  var elementRefsGroup = this.m_loonkInstance._awa.getSvgInstance().findOne(elementRefsGroupId);
                  canvasItemsGroup.add(elementRefsGroup);

                  // Get and remove the element parenting def if contained only that element
                  var currentElementDefsNode = draggedElement.getParentingDefs();
                  if(currentElementDefsNode && currentElementDefsNode.children().length == 1)
                    currentElementDefsNode.remove()

                  // Add the element to new defs bloc
                  var elementDefsNode = this.m_loonkInstance._awa.m_svgInstance.defs(false);
                  canvasItemsGroup.add(elementDefsNode);
                  elementDefsNode.add(draggedElement)
                  ...
                  var parentId = canvasOwnerId.split(GROUP_ID_BODY).join(CLIP_ID_BODY+GROUP_ID_BODY)
                  draggedElement.parentId(parentId); // for scene items refs
          ...
          if( draggedElement.hasClass(INCANVAS_ITEM_CLASS)) // Is in a canvas
          {
            ...
            if(!isPointInCanvas(point,canvasContainer.node) && canvasItemsGroup) 
            {
              ...
              // Get and unparent the element def refs
              var elementRefsGroupId = '#'+draggedElement.attr("id")+DEFREFS_GROUP_ID_BODY;
              var elementRefsGroup = this.m_loonkInstance._awa.getSvgInstance().findOne(elementRefsGroupId);
              draggedElement.canvasOwnerId(null);
              var activeSceneContainer = this.m_loonkInstance._awa.getActiveSceneContainer();
               
              activeSceneContainer.add(elementRefsGroup);
              ...
              // Add the element to new defs bloc
              // var elementDefsNode = this.m_loonkInstance._awa.m_svgInstance.defs(); // Root ?
              var elementDefsNode = activeSceneContainer.getDefs() ? activeSceneContainer.getDefs() : this.m_loonkInstance._awa.m_svgInstance.defs();
              activeSceneContainer.add(elementDefsNode); // parent
              // console.log(elementDefsNode);
              elementDefsNode.add(draggedElement)
