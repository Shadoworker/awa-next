...
*/
import { isCanvas, isCanvasContainer, isGroup, isPointInCanvas } from '../../awa/awa.common.utils';
import { CLIP_ID_BODY, EFFECTOR_GROUP_ID_BODY, GROUP_ID_BODY, INCANVAS_ITEM_CLASS, PLUGGER_ACTIVE_CLASS } from '../../awa/awa.constants';
  ...
  function () {
              ...
              elems.forEach((elem)=>{
                ...
                if(elem.instance && isCanvasContainer(elem.instance))
                {
                  ...
                  if(isPointInCanvas(point,elem)) // Insert
                  {
                    ...
                    var sourceCanvasOwner = source.node._canvasOwnerId;
                    ...
                    if((sourceCanvasOwner != canvasOwner.attr("id")))
                    {
                      ...
                      var basedOnTargetCanvas = source.node._canvasOwnerId || basedOnTarget; // canvas element or the canvas itself
    ...
    // startRelocate : Used to add/remove the element from a canvas
    {
      ...
      value: function startRelocate(ev) {
        ...
        if(this.m_loonkInstance._awa.isElementDraggingStateDragging())
        {
            ...
            if(elem.instance && isCanvasContainer(elem.instance))
            {
              ...
              if(isPointInCanvas(point,elem)) // Insert
              {
                if(!canvasItemsGroup.hasChild(draggedElement))
                {
                  ...
                  draggedElement.node._canvasOwnerId = canvasOwnerId // for refs
          ...
          if( draggedElement.canvasContainer() != undefined) // Is in a canvas
          {
            var canvasContainer = draggedElement.canvasContainer(); // The rect
            var canvasOwner = canvasContainer.instance.parent();
            var canvasItemsGroup = canvasOwner.findOne('.'+CLIP_ID_BODY+GROUP_ID_BODY);
            
            if(!isPointInCanvas(point,canvasContainer) && canvasItemsGroup) 
            {
...

    {
      ...
      value: function endDrag(ev) {
        ...
        // Save
        this.m_loonkInstance._awa.saveProjectChanges();
